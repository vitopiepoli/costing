...
# Assume df is already loaded with the fields:
# 'Service', 'Person_ID', 'County', 'Disability_Severity', 'Cost_Type',
# 'Cost_Description', 'Cost_Value', 'Service_Provider'
df = pd.read_csv("/home/vito/Desktop/Disability/Costing/test.csv") 
st.title("Cost Data Analysis")
...
# Required filters (filtering on Service, Person_ID, County, Disability_Severity)
...
severity_filter = get_filter_values("Disability Severity", df['Disability_Severity'])
...
filtered_df = df[
    ...
    df['Disability_Severity'].isin(severity_filter)
]
...
if filtered_df.empty:
    ...
    # Otherwise, group by only Service, County, and Disability Severity.
    grouping_columns = ['Service', 'County', 'Disability_Severity']
    if provider_filter:
